...
void printPixel (pixel *pix){
  printf("The first pixel element has the following\n\tr = %d\n\tg = %d\n\tb = %d\n",
	 (int) pix->r, (int) pix->g, (int) pix->b);
}

void blurfilter(const int xsize, const int yfrom, const int ysize, pixel* src, const int radius, const double *w){
  int x,y,x2,y2, wi;
  double r,g,b,n, wc;
  pixel dst[MAX_PIXELS];

  for (y=yfrom; y<ysize; y++) {
    for (x=0; x<xsize; x++) {
